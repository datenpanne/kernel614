...
static int huawei_nt51021_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0) {
		dev_err(&ctx->dsi->dev, "Failed to enable panel: %d\n", ret);
	}
...
static int huawei_nt51021_enable(struct drm_panel *panel)
{
	...
	/*ret = mipi_dsi_generic_write(dsi, (u8[]){0x83, 0x00}, 2);
	if (ret < 0) {
		dev_err(dev, "failed to set 8300: %d\n", ret);
		return ret;
	}
	ret = mipi_dsi_generic_write(dsi, (u8[]){0x84, 0x00}, 2);
	if (ret < 0) {
		dev_err(dev, "failed to set 8400: %d\n", ret);
		return ret;
	}
	ret = mipi_dsi_generic_write(dsi, (u8[]){0x9f, 0x66)}, 2);
	if (ret < 0) {
		dev_err(dev, "failed to set 9f66: %d\n", ret);
		return ret;
	}*/
...
static int huawei_nt51021_bl_update_status(struct backlight_device *bl)
{
	...
	ret = huawei_nt51021_set_brightness(dsi, bl->props.brightness);
	if (ret < 0)
		return ret;
...
static int huawei_nt51021_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->backlight = huawei_nt51021_create_backlight(dsi);
